...
public class GlobalExceptionHandler {
    ...
    public Mono<ResponseEntity<ErrorDetails>> handleProductNotFoundException(ProductNotFoundException ex, ServerWebExchange exchange) {
        ErrorDetails errorDetails = new ErrorDetails(LocalDateTime.now(), ex.getMessage(), exchange.getRequest().getPath().toString(), "ENTITY_NOT_FOUND");
        return Mono.just(ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorDetails));
    }

    @ExceptionHandler(ErrorFoundException.class)
    public Mono<ResponseEntity<ErrorDetails>> handleErrorFoundException(ErrorFoundException ex, ServerWebExchange exchange) {
        ErrorDetails errorDetails = new ErrorDetails(LocalDateTime.now(), ex.getMessage(), exchange.getRequest().getPath().toString(), "ERROR_FOUND");
